...
\begin{enumerate}
	...
	All qualified teams can participate in \iterm{Stage~I}. Stage~I comprehends a set of free-form demonstrations that are evaluated based on the achievement of the key concepts detailed in \refchap{chap:stage_I}. These demonstrations can be performed multiple times (See \refsec{rule:score_system}). 
...
% MAURICIO @2017: With the inclusion of SPL only 6 teams per league advance to the second stage (leagues are intended to have 13 teams).
	\item \textbf{Stage~II:} The best \emph{50\% of teams with full integrated capabilities}\footnote{If the total number of teams is less than 12, up to 6 teams may advance to Stage~II} (after Stage~I) advance to \iterm{Stage~II}. Here, a set of story-driven, integrated, complex tests are tested. In order to advance to Stage~II a team must successfully achieve 3 key concepts in Stage~I. \\
	% 2019: removing open challenge, since all tests in Stage I are like an open challenge
...
\begin{enumerate}
	\item \textbf{Stage~I:} The tests are scored by a sum of a Difficulty score (D) and an Execution score (E) with a maximum score of 200 points each. These scores are detailed in \refchap{chap:stage_I}, but are summarized here.
	\begin{enumerate}
		\item \textbf{\iterm{Difficulty Score}:} For each demonstration, the team must choose to show up to 3 key concepts detailed in \refchap{chap:stage_I}. Each key concept has a different difficulty score bound to it, which is granted for every successful demonstration.
		\item \textbf{\iterm{Execution Score}:} It is a score that can be negatively impacted for specific mis-executions from the robot, such as showing less than 3 key concepts in a demonstration, repeating the same key concept from a previous demonstration, the robot not doing anything for a specific amount of time, etc.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 2019: no open challenge anymore
